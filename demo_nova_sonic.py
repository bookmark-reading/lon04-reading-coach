    ...
    try:
        ...
        async with websockets.connect(f"{WS_URL}?token={TOKEN}") as websocket:
            ...
            if response_data.get("type") == "session.created":
                ...
                print(
                    f"✗ Error creating session: {response_data.get('message')}")
                return
            else:
            ...
            if ready_data.get("type") == "session.ready":
                ...
                print(
                    f"⚠️  Expected session.ready, got: {ready_data.get('type')}")
                ...
                try:
                    while running:
                        ...
                        if isinstance(message, bytes):
                            ...
                            print(
                                f"[{timestamp}] #{message_count} 🔊 BINARY: Audio data ({len(message)} bytes)")
                            ...
                            try:
                                ...
                                print(
                                    f"[{timestamp}] #{message_count} 📨 JSON: {json.dumps(data, indent=2)}")

                                if msg_type == "agent.response.started":
                                    print(
                                        f"[{timestamp}]     → 🤖 Agent is responding...")
                                    ...
                                    print(
                                        f"[{timestamp}]     → 🔊 Audio in JSON format")
                                elif msg_type == "agent.response.completed":
                                    print(
                                        f"[{timestamp}]     → ✓ Agent response completed")
                                elif msg_type == "page.completed":
                                    page_num = data.get("page_number")
                                    print(
                                        f"[{timestamp}]     → 📖 Page {page_num} completed!")
                                elif msg_type == "session.ended":
                                    print(
                                        f"[{timestamp}]     → 🛑 Session ended by server")
                                    running = False
                                elif msg_type == "error":
                                    print(
                                        f"[{timestamp}]     → ❌ Error: {data.get('message')}")
                                    print(
                                        f"[{timestamp}]     → Error code: {data.get('code')}")
                            except json.JSONDecodeError:
                                print(
                                    f"[{timestamp}] #{message_count} ⚠️  Non-JSON text: {message[:100]}")
                    ...
                    print(
                        f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Error receiving messages: {e}")
                    import traceback
                    traceback.print_exc()
                    running = False
...
async def test_reading_agent():
    ...
    book = Book(
        ...
        pages=[
            ...
            BookPage(page_number=2,
                     text="So we sat in the house all that cold, cold, wet day."),
            BookPage(page_number=3,
                     text="I sat there with Sally. We sat there, we two."),
            BookPage(page_number=4,
                     text="And I said, How I wish we had something to do!"),
            BookPage(page_number=5,
                     text="Too wet to go out and too cold to play ball."),
        ]
    )
    ...
    print(
        f"✓ Session initialized - speak into your microphone (Ctrl+C to stop)\n")
